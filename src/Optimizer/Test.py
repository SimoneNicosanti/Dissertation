...
def main():
    ...
    opt_params = OptimizationParams(
        ...
        transmission_latency_weight=1.0,
        compute_energy_weight=1.0,
        transmission_energy_weight=1.0,
        device_max_energy=1.0,
        requests_number={model_graph.get_graph_name(): 1},
    )
    ...
    solved_prob_info = OptimizationHandler().optimize(
        model_graph, network_graph, deployment_server, optimization_params=opt_params
    )
    ...
    for edge_id in assignment_graph.get_edges_id():
        print(edge_id.first_node_id.node_name, ">", edge_id.second_node_id.node_name)
