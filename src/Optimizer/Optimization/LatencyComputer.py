...
class LatencyComputer:
    ...
    def __init__(self):
        self.computation_time_cache: dict[
            tuple[NodeId, str], pulp.LpAffineExpression
        ] = {}
        self.transmission_time_cache: dict[
            tuple[NodeId, str], tuple[pulp.LpAffineExpression, pulp.LpAffineExpression]
        ] = {}
    ...
    def compute_latency_cost(
        ...
        for model_graph in model_graphs:
            model_weight = (
                requests_number.get(model_graph.graph["name"]) / total_requests
            )

            model_comp_latency, _ = self.compute_comp_latency_per_model(
                model_graph, network_graph, node_ass_vars, server_execution_profile_pool
            )
            model_trans_latency, _ = self.compute_trans_latency_per_model(
                model_graph, network_graph, tensor_ass_vars
            )

            normalization_factor = (
                1  # max(max_model_comp_latency, max_model_trans_latency)
            )
    ...
    def compute_comp_latency_per_model(
        ...
        for net_node_id in network_graph.nodes:
            node_comp_latency_per_model, node_max_comp_latency_per_model = (
                self.compute_model_comp_latency_per_node(
                    model_graph,
                    network_graph,
                    node_ass_vars,
                    net_node_id,
    ...
    def compute_trans_latency_per_model(
        ...
        for net_node_id in network_graph.nodes:
            ...
            ) = self.compute_model_trans_latency_per_node(
    ...
    def compute_model_comp_latency_per_node(
        ...
        if (net_node_id, model_name) in self.computation_time_cache:
            return self.computation_time_cache[net_node_id, model_name], 0
        ...
        for mod_node_id in model_graph.nodes:
            ...
            time_expr, layer_max_comp_time = LatencyComputer.__get_computation_time(
                model_graph,
                mod_node_id,
                net_node_id,
        ...
        self.computation_time_cache[net_node_id, model_name] = pulp.lpSum(sum_elems)
    ...
    def compute_model_trans_latency_per_node(
        ...
        model_name = model_graph.graph["name"]
        if (net_node_id, model_name) in self.transmission_time_cache:
            return (
                self.transmission_time_cache[net_node_id, model_name][0],
                self.transmission_time_cache[net_node_id, model_name][1],
                0,
            )
        ...
        for tensor_name in tensors_dict.keys():
            for net_edge_id in network_graph.edges:
                if net_edge_id[0] == net_node_id:
                    ...
                    trans_time_expr, layer_max_trans_time = (
                        LatencyComputer.__get_transmission_time(
                            model_graph,
                            network_graph,
                            tensor_name,
                            net_edge_id,
        ...
        self.transmission_time_cache[net_node_id, model_name] = (
    ...
    def __get_computation_time(
        ...
        not_quant_ass_key = NodeAssKey(
            mod_node_id, net_node_id, model_graph.graph["name"]
        )
        not_quant_time = model_execution_profile.get_not_quantized_layer_time(
            mod_node_id
        )
        ...
        if model_graph.nodes[mod_node_id].get(ModelNodeInfo.QUANTIZABLE, False):
            ...
            max_comp_time = max(max_comp_time, 0)
