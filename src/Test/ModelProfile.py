...
def profile_nodes_edges(model_name: str):
    ...
    model_names = build_profile_list(model_name)

    model_profiler = get_profiler_stub()

    dataframe = pd.DataFrame(columns=["model_name", "num_nodes", "num_edges"])

    for curr_model_name in model_names:

        profile_request = ProfileRequest(
            model_id=ModelId(model_name=curr_model_name), profile_regression=False
        )
