...
def main():
    ...
    device_plan_gen_df = (
        ...
        .fillna(-1)[group_cols + ["latency_value", "energy_value"]]
        .groupby(group_cols)
        .mean()
        .reset_index()
    )
    ...
    models = ["yolo11x-seg"]
    for model in models:
        for dev_cpus in device_edge_plan_gen_df["device_cpus"].unique():
            for edge_cpus in device_edge_plan_gen_df["edge_cpus"].unique():
                fig: plt.Figure
                axes: list[list[plt.Axes]]
                fig, axes = plt.subplots(
                    figsize=(14, 7),
                    nrows=2,
                    ncols=len(device_edge_plan_gen_df["latency_weight"].unique()),
                    sharey="row",
                )

                latency_weights = device_edge_plan_gen_df["latency_weight"].unique()
                ...
                for i, lw in enumerate(latency_weights):
                    ...
                    device_edge_plan_gen_df_i = device_edge_plan_gen_df[
                        (device_edge_plan_gen_df["latency_weight"] == lw)
                    ...
                    device_edge_cloud_plan_gen_df_i = device_edge_cloud_plan_gen_df[
                        (device_edge_cloud_plan_gen_df["latency_weight"] == lw)
                    ...
                    curr_ax.plot(
                        ...
                        device_plan_gen_df_i["latency_value"],
                        label="device",
                        marker="o",
                    )
                    curr_ax.plot(
                        device_edge_plan_gen_df_i["max_noises"].unique(),
                        device_edge_plan_gen_df_i["latency_value"],
                        label="device + edge",
                        marker="o",
                    )
                    curr_ax.plot(
                        device_edge_cloud_plan_gen_df_i["max_noises"].unique(),
                        device_edge_cloud_plan_gen_df_i["latency_value"],
                    ...
                    merged_df: pd.DataFrame = merged_df.rename(
                        columns={
                            ...
                            "latency_value_device": "Device",
                            "latency_value_device_edge": "Device+Edge",
                            "latency_value": "Device+Edge+Cloud",
                        }
                    )
                ...
                energy_weights = np.sort(
                    device_edge_plan_gen_df["energy_weight"].unique()
                )[::-1]
                ...
                fig.suptitle(f"Theory Comparison. Model: {model}", fontsize=13)

                plt.tight_layout()
                plt.savefig(
                    f"../Images/Theory_Comparison/theory_comparison_{model}_{dev_cpus}_{edge_cpus}.png"
                )
                plt.savefig(
                    f"../Images/Theory_Comparison/theory_comparison_{model}_{dev_cpus}_{edge_cpus}.svg"
                )
